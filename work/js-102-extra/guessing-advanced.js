...
const wordInfo = {
  ...
  function thinkAbout( wordInfo ) {
    ...
    let pool = new Set();
    for(let i = 0;i < wordInfo.allWords.length;i++){
        pool.add(wordInfo.allWords[i]);
    }
    return pool;
    // EDIT ABOVE THIS
  }
  
  function pickGuess( wordInfo, history ) { // corner case: if word can not jump from changing only one letter
    ...
    // EDIT BELOW THIS
    let iterator = wordInfo.magic.values();
    let value = iterator.next().value;
    wordInfo.magic.delete(value);
    return value;
    //"HIRST" "HIKRS"
    // EDIT ABOVE THIS
  }
  ...
  function compareLetters( guess, wordInfo ) {
    ...
    let setIter = wordInfo.magic.values();
    let value = setIter.next().value;
    while(value !== undefined){
        let tmp = common(guess,value);
        if(tmp != result.similar){
            wordInfo.magic.delete(value);
        }
        value = setIter.next().value;
    }
    return result;
    // EDIT ABOVE THIS
  }
